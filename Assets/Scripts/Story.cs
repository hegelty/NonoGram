...
public class Story : MonoBehaviour, IPointerDownHandler
{
    ...
    private void LoadStory(int storyId)
    {
        ...
        var storyFile = Resources.Load<TextAsset>("Stories/story_" + storyId);
    ...
    {
        if(eventData.button == PointerEventData.InputButton.Left && _storyId >= 0) //좌클릭이면서 스토리 번호가 설정되어 있을때
        {
